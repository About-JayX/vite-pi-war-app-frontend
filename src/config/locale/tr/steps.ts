export default {
  openPiWar: "Pi War'ı Aç",
  yearAgo: "Yıl Önce",
  continue: "Devam Et",
  steps1: {
    text1: "Pi War 𝜋: Web3 Destekli Telegram Oyunu.",
    text2: "Lütfen Telegram ayarlarında kullanıcı adınızı düzenleyin",
  },
  steps2: {
    title: "Hesabınız Kontrol Ediliyor",
  },
  steps3: {
    title: "Evreni Keşfedin!",
    text1: "Telegram'a katıldınız",
    text2: ["Hesap numaranız #", "Telegram kullanıcılarının %85'inde yer alıyorsunuz"],
  },
  steps4: {
    title: "Pi War 𝜋'ya Hoş Geldiniz!",
    text1: "İşte 𝜋PIS ödülleriniz",
    text2: "Telegram'ı kullandığınız için teşekkürler",
  },
};
